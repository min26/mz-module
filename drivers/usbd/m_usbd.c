...
/*
 * Instantiate a context named m_usbd using the default USB device
 * controller, the Zephyr project vendor ID, and the product ID.
 * Zephyr project vendor ID must not be used outside of Zephyr.
 */
USBD_DEVICE_DEFINE(m_usbd,
		   DEVICE_DT_GET(DT_NODELABEL(zephyr_udc0)),
		   CONFIG_M_USBD_VID, CONFIG_M_USBD_PID);
/* doc device instantiation end */

/* doc string instantiation start */
USBD_DESC_LANG_DEFINE(m_lang);
USBD_DESC_MANUFACTURER_DEFINE(m_mfr, CONFIG_M_USBD_MANUFACTURER);
USBD_DESC_PRODUCT_DEFINE(m_product, CONFIG_M_USBD_PRODUCT);
IF_ENABLED(CONFIG_HWINFO, (USBD_DESC_SERIAL_NUMBER_DEFINE(m_sn)));
...
static const uint8_t attributes = (IS_ENABLED(CONFIG_M_USBD_SELF_POWERED) ?
				   USB_SCD_SELF_POWERED : 0) |
				  (IS_ENABLED(CONFIG_M_USBD_REMOTE_WAKEUP) ?
				   USB_SCD_REMOTE_WAKEUP : 0);

/* Full speed configuration */
USBD_CONFIGURATION_DEFINE(m_fs_config,
			  attributes,
			  CONFIG_M_USBD_MAX_POWER, &fs_cfg_desc);

/* High speed configuration */
USBD_CONFIGURATION_DEFINE(m_hs_config,
			  attributes,
			  CONFIG_M_USBD_MAX_POWER, &hs_cfg_desc);
/* doc configuration instantiation end */

#if CONFIG_M_USBD_20_EXTENSION_DESC
/*
...
struct usbd_context *m_usbd_setup_device(usbd_msg_cb_t msg_cb)
{
	...
	err = usbd_add_descriptor(&m_usbd, &m_lang);
	if (err) {
		LOG_ERR("Failed to initialize language descriptor (%d)", err);
		return NULL;
	}

	err = usbd_add_descriptor(&m_usbd, &m_mfr);
	if (err) {
		LOG_ERR("Failed to initialize manufacturer descriptor (%d)", err);
		return NULL;
	}

	err = usbd_add_descriptor(&m_usbd, &m_product);
	if (err) {
	...
	IF_ENABLED(CONFIG_HWINFO, (
		err = usbd_add_descriptor(&m_usbd, &m_sn);
	))
	if (err) {
	...
	if (USBD_SUPPORTS_HIGH_SPEED && usbd_caps_speed(&m_usbd) == USBD_SPEED_HS) {
		err = usbd_add_configuration(&m_usbd, USBD_SPEED_HS, &m_hs_config);
		if (err) {
